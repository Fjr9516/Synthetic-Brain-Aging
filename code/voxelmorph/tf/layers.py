...
class VecInt(Layer):
    ...
    def __init__(self, indexing='ij', method='ss', int_steps=7, out_time_pt=1, 
        ...
        assert indexing in ['ij', 'xy'], "indexing has to be 'ij' (matrix) or 'xy' (cartesian)"
        self.indexing = indexing # ij
        self.method = method # 'ode'
        self.int_steps = int_steps # 10
        self.inshape = None
        self.out_time_pt = out_time_pt # 1
    ...
    def call(self, inputs):
        print('--------------------------Call function of VecInt----------------------------------------------/n')
        if not isinstance(inputs, (list, tuple)):
            print('-- In if not isinstance(inputs, (list, tuple)) --')
            print('-- inputs = [inputs] --')
            inputs = [inputs]
            print('-- type of inputs:{}, len of inputs: {} --'.format(type(inputs), len(inputs)))  # list, len = 1
        loc_shift = inputs[0] # 80x80x96x3
        print('-- loc_shift = inputs[0] --')
        print('-- type of loc_shift:{}, shape of it is: {} --'.format(type(loc_shift),loc_shift.shape))

        # necessary for multi_gpu models...
        loc_shift = K.reshape(loc_shift, [-1, *self.inshape[1:]])
        print('-- loc_shift = K.reshape(loc_shift, [-1, *self.inshape[1:]]) --')
        print('-- type of loc_shift:{}, shape of it is: {} --'.format(type(loc_shift), loc_shift.shape))
        if hasattr(inputs[0], '_keras_shape'):
            loc_shift._keras_shape = inputs[0]._keras_shape
            print('-- In if hasattr(inputs[0], _keras_shape) --')
            print('-- loc_shift._keras_shape:{} --'.format(loc_shift._keras_shape))
        print('-- Skip if hasattr(inputs[0], _keras_shape) --')
        ...
        # map transform across batch
        print('-- type of [loc_shift] + inputs[1:]:{}, len of it is:{} --'.format(type([loc_shift] + inputs[1:]), len(
           [loc_shift] + inputs[1:])))  # list, 1
        print(inputs[1:])
        out = tf.map_fn(self._single_int, [loc_shift] + inputs[1:], dtype=tf.float32)
        if hasattr(inputs[0], '_keras_shape'):
            print('-- In hasattr(inputs[0], _keras_shape) --')
            out._keras_shape = inputs[0]._keras_shape
            print('-- out._keras_shape:', out._keras_shape)
        print('-- type of out:{}, shape of it is:{} --'.format(type(out), out.shape))  # tensor, (None, 16, 16, 2)
        return out

    def _single_int(self, inputs):
        print('Calling _single_int')
        vel = inputs[0]
        print('vel type and shape are {} and {}'.format(type(vel),vel.shape))
        out_time_pt = self.out_time_pt
        print(out_time_pt)
        if len(inputs) == 2:
    ...
    def call(self, inputs):
        ...
        if self.is_affine:
            ...
            if self.zoom_factor < 1:
                # resize
                trf = ne.layers.Resize(self.zoom_factor, name=self.name + '_resize', dtype= 'float32')(trf)
                ...
                return ne.layers.Resize(self.zoom_factor, name=self.name + '_resize', dtype= 'float32')(trf)
