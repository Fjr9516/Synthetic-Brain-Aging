...
class VxmDense_AMP(LoadableModel):
    """
    VoxelMorph network for (unsupervised) nonlinear registration between two images.
    """
    ...
    def __init__(self,
        ...
        if not unet_half_res:
            # optionally resize for integration
            if int_steps > 0 and int_downsize > 1:
                flow = layers.RescaleTransform(1 / int_downsize, name='flow_resize',dtype='float32')(flow)#TODO
        ...
        # integrate to produce diffeomorphic warp (i.e. treat flow as a stationary velocity field)
        if int_steps > 0:
            pos_flow = layers.VecInt(method='ss', name='flow_int', int_steps=int_steps, dtype= 'float32')(pos_flow)# TODO
            if bidir:
                neg_flow = layers.VecInt(method='ss', name='neg_flow_int', int_steps=int_steps, dtype= 'float32')(neg_flow)# TODO

            # resize to final resolution
            if int_downsize > 1:
                pos_flow = layers.RescaleTransform(int_downsize, name='diffflow',dtype='float32')(pos_flow)# TODO
                if bidir:
                    neg_flow = layers.RescaleTransform(int_downsize, name='neg_diffflow',dtype='float32')(neg_flow)# TODO

        # warp image with flow field
        y_source = layers.SpatialTransformer(interp_method='linear', indexing='ij', name='transformer', dtype= 'float32')([source, pos_flow])
        if bidir:
            y_target = layers.SpatialTransformer(interp_method='linear', indexing='ij', name='neg_transformer', dtype= 'float32')([target, neg_flow])
    ...
    def apply_transform(self, src, trg, img, interp_method='linear'):
        ...
        warp_model = self.get_registration_model()
        img_input = tf.keras.Input(shape=img.shape[1:])
        y_img = layers.SpatialTransformer(interp_method=interp_method, dtype= 'float32')([img_input, warp_model.output])
        return tf.keras.Model(warp_model.inputs + [img_input], y_img).predict([src, trg, img])
...
class ConditionalTemplateCreation_AMP(LoadableModel):
    """
    VoxelMorph network to generate an conditional template image.
    """
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        # warp model
        vxm_model = VxmDense_AMP(inshape, nb_unet_features=nb_unet_features, input_model=warp_input_model, **kwargs)
        ...
        if use_mean_stream:
            # get mean stream from negative flow
            mean_stream = ne.layers.MeanStream(name='mean_stream', cap=mean_cap, dtype='float32')(neg_flow) # TODO
            outputs = [y_source, mean_stream, pos_flow, pos_flow]
        else:
            outputs = [y_source, pos_flow, pos_flow]
